...
const { merge, zipObject, intersection, cloneDeep, compact, uniq } = require('lodash');
const walkSync = require('walk-sync');
const EOL = require('os').EOL;
const logger = require('heimdalljs-logger')('layer-gen:blueprint');
const normalizeEntityName = require('./normalise-entity-name');
...
*/
const Blueprint = CoreObject.extend({
  availableOptions: [],
  ...
  */
  _writeStatusToUI(chalkColor, keyword, message) {
    if (this.ui) {
      console.log(`  ${chalkColor(keyword)} ${message}`);
    }
  },
  ...
  */
  uninstall(options) {
    ...
    if (this.dryRun) {
      console.log(chalk.yellow('You specified the `dry-run` flag, so no files will be deleted.'));
    }
  ...
  addAddonsToProject(options) {
    ...
    if (packages && packages.length) {
      taskOptions.packages = packages.map((pkg) => {
        ...
        if (!pkg.name) {
          throw new Error('You must provide a package `name` to addAddonsToProject');
        }
      ...
      throw new Error('You must provide package to addAddonsToProject');
    }
...
/**
  ...
  @param {Boolean} [options.ignoreMissing] Throw a `Error` if a
...
Blueprint.lookup = function (name, options) {
  ...
  if (blueprintMap.has(name)) {
    return Blueprint.load(blueprintMap.get(name).path);
  }
...
function loadBlueprints(path, blueprintMap, { throwOnClash = true } = {}) {
  ...
  if (existsSync(fullPath)) {
    const blurprints = readdirSync(fullPath).map((fileName) => {
      ...
      if (fs.lstatSync(blueprintPath).isDirectory()) {
        ...
        blueprintMap.set(fileName, {path: blueprintPath, overridden: blueprintMap.has(fileName)})
      }
    });
  }
}
...
function loadBlueprint(path, blueprintMap, { throwOnClash = true, throwOnMissing = false } = {}) {
  if (fs.lstatSync(path).isDirectory()) {
    ...
    // todo figure out what to do in this map
    blueprintMap.set(blueprintName, {path: path, overridden: blueprintMap.has(blueprintName)})
...
/**
  ...
  @return {Map}
*/
function generateBlueprintMap(lookupPaths) {
  let blueprintMap = new Map();

  // get blueprints from all depenedencies
  ...
  allDependencies.forEach((dep) => {
    const depJsonFile = require.resolve(`${dep}/package.json`,{paths: [process.cwd()]});
    ...
    if (blueprintConfig) {
      for (const blueprintName in blueprintConfig) {
        ...
        loadBlueprint(join(dirname(resolvedDep), 'blueprints', blueprintName), blueprintMap, { throwOnClash: false, throwOnMissing: true });
      }
    }
  });
