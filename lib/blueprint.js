...
function generateLookupPaths(lookupPaths) {
  ...
  const localBlueprintsDir = path.join(path.dirname(pkgUp.sync()), 'blueprints');

  if (existsSync(localBlueprintsDir)) {
    lookupPaths.push(localBlueprintsDir);
  }

  // TODO add dependencies' blueprints if they exist
